...
const khmerFonts = [
    { name: "AKbalthom Freehand", value: "'AKbalthom Freehand', sans-serif" },
...
export default function ClientDashboard() {
  ...
  const saveWeddingInfo = async () => {
    if (clientEmail) {
      ...
      // Create/update preview invitation
      if (previewSlug) {
      ...
      // Also save to invites collection for each guest slug
      const guestSnap = await getDocs(collection(db, 'Users', clientEmail, 'Guests'));
      const promises = guestSnap.docs.map(async (guestDoc) => {
        const guestData = guestDoc.data();
        if (guestData.slug) {
          await setDoc(doc(db, 'invites', guestData.slug), {
            ...weddingInfo,
      ...
      setSuccess("Wedding info updated and synced to all invitations!");
      setTimeout(() => setSuccess(""), 1500);
    }
  };
  ...
  const addGuest = async (e: React.FormEvent) => {
    ...
    if (clientEmail && guestName) {
      ...
      await addDoc(collection(db, 'Users', clientEmail, 'Guests'), { name: guestName, slug, status: 'pending' });
  ...
  const handleEditGuest = async (guest: any) => {
    const newStatus = window.prompt("Update status (going / not-going / pending):", guest.status);
    if (newStatus) {
      await updateDoc(doc(db, 'Users', clientEmail!, 'Guests', guest.id), { status: newStatus });
                      ...
                      {guests.map((guest) => (
                        <Tr key={guest.id} _hover={{ bg: 'gray.50' }}>
                          ...
                          <Td>
                            <Badge 
                              colorScheme={
                                guest.status === 'going' ? 'green' : 
                                guest.status === 'not-going' ? 'red' : 'yellow'
                              }
                            >
